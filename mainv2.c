...
char SCAN_TO_CHAR(u32 scan)
{
	...
	switch(scan)
	{
		...
		case 89 : result = '^'; break;		// shift


		// this block is really ugly, i Know why I need it sort of , but for whatever reason
		// the hardware reports strange values for F0 (the break key) so I need all these to make sure that a bumch of junk
		// output doesnt get through, and I also need these characters for state transitions
		case 63 : result = '}';
		case 203 : result = '}';
		case 191 : result = '}';
		case 211 : result = '}';
...
//int * coordcheck(int CURR[2], int MAX[2])
//{
//	if (coordCUR[0] >= coordMAX[0])
//	{
//		coordCUR[1]++;
//		coordCUR[0] = 0;
//		//printf("\n\r");
//	}
//	else if (coordCUR[0]< 0)
//	{
//		coordCUR[0] = 0;
//	}
//
//	if (coordCUR[1] >= coordMAX[1])
//	{
//		coordCUR[1] = 0;
//		coordCUR[0] = 0;
//	}
...
int main()
{
    ...
    // zero is in case we need a proper zero for something, and make sure it is of type int
    //static u32 zero = 0;

    static u8 coordMAX[2] = {80, 40};
    u8 coordCUR[2] = {0, 0};
...
//			printf("%x \n\r",scancode[0]);
			...
			// letters and numbers are handled reguarly, specials are handled specially
...
//					printf("\n\r");
			...
			// this is to catch } and - characters, which should not be printed under any circumstance
			if ((key == 125) | (key == 45))
			{
				continue;
			}

			data_sent = key | (coordCUR[1] << 8) | (coordCUR[0] << 16);
			//write data
			printf("Row: %d, Column: %d\n\r", coordCUR[1], coordCUR[0]);
			printf("Result: %c [%d] \r\n", key, key);
			printf("Data: %x \n\r", data_sent);
			//printf("%c", key);
